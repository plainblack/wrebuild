...
for /f "usebackq tokens=1,3" %%d in (`reg query "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v PATH`) do (
  if "%%d"=="PATH" set globalpath=";%%e;"
)
...
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v "PATH" /t REG_SZ /d %globalpath% /f
